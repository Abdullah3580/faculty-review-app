...
export default async function FacultyProfilePage(props: Props) {
  ...
  const averageRating =
    totalReviews > 0
      ? (
          faculty.reviews.reduce((acc, r) => acc + r.rating, 0) / totalReviews
        ).toFixed(1)
      : "0.0";
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <Link
                              href={`/student/${review.user.id}`}
                              className="font-bold text-gray-800 dark:text-white hover:text-indigo-500"
                            >
                              @{review.user?.nickname || "Anonymous"}
                            </Link>
                            <UserBadge
                              reviewCount={review.user?._count?.reviews || 0}
